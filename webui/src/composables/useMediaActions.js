...
export function useMediaActions(path, screenshotVariant, screenshotSubtitleMode, screenshotCount, screenshotCaptureMode, hasInput) {
    ...
    const runDownloadTask = async ({ jobId = "" } = {}) => {
        ...
        try {
            ...
            let trackedTask = baseTask;
            if (jobId === "") {
                const job = await createScreenshotJob(path.value.trim(), screenshotVariant.value, screenshotSubtitleMode.value, screenshotCount.value, screenshotCaptureMode.value, "zip");
    ...
    const runLinkTask = async ({ action, jobId = "" } = {}) => {
        ...
        try {
            ...
            let trackedTask = baseTask;
            if (jobId === "") {
                const job = await createScreenshotJob(path.value.trim(), screenshotVariant.value, screenshotSubtitleMode.value, screenshotCount.value, screenshotCaptureMode.value, "links");
